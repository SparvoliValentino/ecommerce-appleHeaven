    ...
    <header className='w-full h-[100px] my-6'>
      ...
      {menuOpen && (
        <div className='fixed inset-0 bg-black bg-opacity-90 z-50 '>
          <div className='min-h-[600px] max-h-[800px] flex flex-col justify-center items-center '>
            ...
            <ul className='text-white text-center space-y-6 h-[400px] flex flex-col justify-evenly'>
              <a href='/Home' onClick={toggleMenu}><li className='bg-gray-100 text-black shadow-custom-inset rounded-full p-4 text-2xl'>Home</li></a>
              <a href='/product' onClick={toggleMenu}><li className='bg-gray-100 text-black shadow-custom-inset rounded-full p-4 text-2xl'>Products</li></a>
              {userData?.token ? (
                <a href='/dashboard' onClick={toggleMenu}><li className='bg-gray-100 text-black shadow-custom-inset rounded-full p-4 text-2xl'>Account settings</li></a>
              ) : (
                <a href='/login' onClick={toggleMenu}><li className='bg-gray-100 text-black shadow-custom-inset rounded-full p-4 text-2xl'>Register</li></a>
              )}
              {userData?.token ? (
                <a href='/order/cart' onClick={toggleMenu}><li className='bg-gray-100 text-black shadow-custom-inset rounded-full p-4 text-2xl'>Cart</li></a>
              ) : (
                <a href='/login' onClick={toggleMenu}><li className='bg-gray-100 text-black shadow-custom-inset rounded-full p-4 text-2xl'>Cart</li></a>
              )}
            </ul>
          </div>
        </div>
